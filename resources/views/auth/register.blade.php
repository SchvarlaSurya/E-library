...
<head>
    ...
    <title>Athena — Create Account</title>
    ...
    <div class="min-h-screen flex items-center justify-center px-6 page-enter relative overflow-hidden">
        ...
        <div class="w-full max-w-[400px] relative z-10 space-y-8 bg-white/70 p-8 sm:p-10 rounded-[2rem] border border-white shadow-2xl backdrop-blur-xl">
            <div class="text-center space-y-2">
                ...
                <h2 class="text-3xl font-black tracking-tight text-zinc-900 mb-2">Buat Akun</h2>
                <p class="text-sm font-medium text-zinc-500">Bergabunglah dengan ReadSpace hari ini.</p>
            </div>

            <div id="note-message" class="hidden text-amber-700 text-[11px] font-bold tracking-wide bg-amber-50 p-4 rounded-xl border border-amber-200 flex items-center gap-2 shadow-sm">
                <i data-lucide="info" class="w-4 h-4 shrink-0"></i>
                <span>Anda belum login. Silakan mendaftar terlebih dahulu.</span>
            </div>

            <div id="register-form" class="space-y-4 pt-2">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[11px] uppercase tracking-widest font-bold text-zinc-500 pl-1">First Name</label>
                        <input type="text" id="first_name" required placeholder="John" 
                            class="w-full bg-white/50 border border-black/5 rounded-2xl px-5 py-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-[#8b7355] focus:ring-1 focus:ring-[#8b7355] transition-all shadow-sm">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[11px] uppercase tracking-widest font-bold text-zinc-500 pl-1">Last Name</label>
                ...
                <div class="space-y-1.5">
                    <label class="text-[11px] uppercase tracking-widest font-bold text-zinc-500 pl-1">Password</label>
                    <input type="password" id="password" required placeholder="••••••••" 
                        class="w-full bg-white/50 border border-black/5 rounded-2xl px-5 py-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-[#8b7355] focus:ring-1 focus:ring-[#8b7355] transition-all shadow-sm">
                </div>

                <button id="submit-register" class="w-full bg-[#1a1816] text-[#fdfbf7] py-4 rounded-2xl text-sm font-bold shadow-xl shadow-black/10 hover:scale-[1.02] transition-all mt-4 active:scale-[0.98] flex items-center justify-center gap-2">
            ...
            <div class="relative my-8 group">
                ...
                <div class="relative flex justify-center text-[10px] uppercase tracking-widest">
                    <span class="bg-[#fcfaf7] px-4 font-bold text-zinc-400">Atau mendaftar dengan</span>
                </div>
            </div>

            <div class="grid grid-cols-2 gap-3">
                <button type="button" onclick="signUpWith('oauth_google')" class="flex items-center justify-center gap-2 py-3.5 bg-white border border-black/5 rounded-2xl hover:bg-zinc-50 hover:border-black/10 transition-all active:scale-95 text-xs font-bold text-zinc-600 shadow-sm">
                    <img src="https://www.svgrepo.com/show/475656/google-color.svg" class="w-4 h-4" alt="Google">
                    Google
                </button>
                <button type="button" onclick="signUpWith('oauth_github')" class="flex items-center justify-center gap-2 py-3.5 bg-white border border-black/5 rounded-2xl hover:bg-zinc-50 hover:border-black/10 transition-all active:scale-95 text-xs font-bold text-zinc-600 shadow-sm">
            ...
            <p class="text-center text-xs font-medium text-zinc-500 mt-2">
                Sudah punya akun? 
                <a href="/login" class="text-[#8b7355] font-bold hover:underline decoration-2 underline-offset-4">Masuk</a>
            </p>
        </div>
    </div>
...
<script>
    ...
    window.addEventListener('load', async function() {
        try {
            if (window.Clerk) {
                ...
                // Show note message if URL has ?note=belum-login
                const urlParams = new URLSearchParams(window.location.search);
                if (urlParams.get('note') === 'belum-login') {
    ...
    async function handleRegister() {
        ...
        try {
            ...
            if (signUp.status === 'complete') {
                ...
                console.log("Mungkin butuh verifikasi email:", signUp.status);
                errorEl.innerText = "Please check your email to verify your account.";
            ...
            const exists = clkErrors.find(e => e.code === 'form_identifier_exists');
            
            if (exists) {
                // Redirect user to login with note
                window.location.href = '/login?note=sudah-punya-akun';
                return;
            }

            document.getElementById('error-text').innerText = clkErrors.length > 0 ? clkErrors[0].longMessage : "AUTH_ERROR: Registration failed.";
            errorEl.classList.remove('hidden');
        } finally {
    ...
    async function signUpWith(provider) {
        try {
            ...
            window.Clerk.openSignUp({ strategy: provider });
        }
    }
</script>
</body>
